...
If we add new functions, we'll expand this function so that the user is
presented with new options.
*/
void InventorDB::dbMenu() {
	...
	const char *options[] = {"Reload DB From File",
...
void InventorDB::readFile(const char fileName[]) {
	...
	if (head == nullptr) {
		cerr << "There is no inventor data in " << fileName
